...
SPPose plan_velocity(SPPose *current, SPPose *target) {
    ...
    next_goal.velo.x = sub_plan_velocity(current->velo.x, target->velo.x);
...
double sub_plan_velocity(const double cur, const double tar, const double diff) {
	double next;
	next = tar;
	next = smooth_start_clamper
}

double smooth_start_clamper(double planned_velocity, double max_acceleration, double current_velocity, double time_step) {
