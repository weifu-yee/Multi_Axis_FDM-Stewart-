...
void reset_pushers_to_home(void) {
	...
	while (cnt_5 < time_points[0]) {
		for (int i = 1; i <= 6; ++i) {
			pusher[i].goal_pulse = reset_pulse;
			pusher[i].dir = -1.0;
		}
	//	actuate_pushers();
	...
	leg_un_origin = 6;
	update_pusher_encoders(); //clean the buffer
	update_from_sensor();
	...
	while (leg_un_origin) {
		...
		for (int i = 1; i <= 6; ++i) {
			...
			else if ((So - current_lengths[i]) < length_to_slowdown) {
				pusher[i].goal_pulse = 100.0;
				pusher[i].dir = 1.0;
			}
		}
...
void update_pushers_PWM(const double diff_lengths[6]) {
   ...
   for (int i = 1; i <= 6; ++i) {
       ...
       if (pusher[i].u >= 0.0)	pusher[i].dir = 1;
       else pusher[i].dir = -1;
